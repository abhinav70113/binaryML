...
if os.path.exists(list_of_dicts_dir):
    ...
    default_hyperparameters = {
        ...
        'batch_size': 1200,
        'index':0,
        'deep_layer_size':str([64,192,256]),
        'epochs':20000,
        'patience':400,
        'input_shape':(400,1),
    ...
    search_space = {
        'num_deep_layers': [1,2,4,6,8,10,12],
        'initial_learning_rate': [round(i, 5) for i in np.logspace(-4, -3, 20)],
        'decay_rate': [round(i, 2) for i in np.arange(0.5, 0.8, 0.05)],
        'batch_size': [400,800],
        'num_cnn_layers': [2,4,6,8,10,12],
        'batch_normalization':[True,False],
        'attention_type':['multi_head','simple'],
    ...
    for i,ele in enumerate(list_of_dicts[:9999]): # set maimum number of trials that can be tested to 10000
        ...
        for j in range(150):
            ...
            if j < 50:
                ...
                last_filter_size.append(start*ele['num_cnn_layers'])
                ...
                filter_list = random.sample(range(8, 257*ele['num_cnn_layers'], 8), ele['num_cnn_layers'])
                list_conv1d_filters.append(str(filter_list))
                last_filter_size.append(filter_list[-1])
        ...
        for i in range(12):
            ...
            filter_list_number = np.random.randint(0,150)
            ele['conv1d_filters'] = list_conv1d_filters[filter_list_number]
            attention_head_number_found = False
            while not attention_head_number_found:
                attention_head_number = np.random.randint(1,17)
                if last_filter_size[filter_list_number] % attention_head_number == 0:
                    attention_head_number_found = True
            ele['num_attention_heads'] = attention_head_number
            ele['num_key_dims'] = last_filter_size[filter_list_number] // attention_head_number
            ele['epochs'] = 20000
            ele['patience'] = 400
