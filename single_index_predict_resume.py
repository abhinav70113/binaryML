...
try:
    previous_job_id = sys.argv[5]
    resume_training_previous_job_id = int(previous_job_id)
    resume_training = True
    best_model_name_resume = f'{cur_dir}models/f_predict_{model_type}_{resume_training_previous_job_id}_'
    myexecute(f'rsync -Pav -q {root_dir}models/f_predict_{model_type}_{resume_training_previous_job_id}_checkpoint.h5 {cur_dir}models/')
except IndexError:
...
if model_type == 'cnn':
    list_of_dicts = json.load(open(f'/hercules/scratch/atya/BinaryML/hyperparameter_tuning/cnn_f_fine/list_of_dicts.json','r'))

elif model_type == 'attention':
    list_of_dicts = json.load(open(f'/hercules/scratch/atya/BinaryML/hyperparameter_tuning/attention_z_fine/list_of_dicts.json','r'))
